...
try {
 ...
 await page.getByRole('button',{name:'a.example.com',exact:true}).click();
 assert.deepEqual((await expression()).rules,[{field:'domain',op:'in',values:['a.example.com']}]);
 assert.equal((await applied()).get('domain'),null);
 await page.getByRole('button',{name:'编辑筛选',exact:true}).click();
 assert.equal(await page.getByLabel('条件 1 字段',{exact:true}).inputValue(),'domain');
 await page.getByLabel('条件 1 值',{exact:true}).fill('edited.example.com');
 await page.getByRole('button',{name:'应用筛选',exact:true}).click();
 await chips.getByTitle('域名: edited.example.com',{exact:true}).waitFor();
 ...
 assert.deepEqual((await expression()).rules[0].values,['']);
 await page.getByRole('button',{name:'编辑筛选',exact:true}).click();
 await page.getByLabel('条件组合',{exact:true}).selectOption('any');
 await page.getByRole('button',{name:'应用筛选',exact:true}).click();
 await page.getByLabel('统计维度',{exact:true}).selectOption('domain');
 await page.getByRole('button',{name:'a.example.com',exact:true}).click();
 assert.equal((await expression()).match,'any');
 assert.equal((await expression()).rules.length,2);
 await page.getByLabel('统计维度',{exact:true}).selectOption('domain');
 await page.getByRole('button',{name:'a.example.com',exact:true}).click();
 assert.equal((await expression()).rules.length,2);
 await page.getByRole('button',{name:'编辑筛选',exact:true}).click();
