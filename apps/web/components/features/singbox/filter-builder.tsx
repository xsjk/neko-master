...
type FilterDraft = { match: NativeFilterExpression['match']; rules: (NativeFilterRule & { matchUnknown: boolean })[] };
function editDraft(value: NativeFilterExpression): FilterDraft {
  return { ...value, rules: value.rules.map(rule => ({ ...rule, values: rule.values.filter(text => text !== ''), matchUnknown: rule.values.includes('') })) };
}
function appliedDraft(draft: FilterDraft): NativeFilterExpression {
  return { ...draft, rules: draft.rules.map(({ matchUnknown, ...rule }) => {
    if (rule.op !== 'in' && rule.op !== 'notIn') return rule;
    const values = [...new Set(rule.values.filter(text => text.trim() !== ''))];
    return { ...rule, values: matchUnknown ? [...values, ''] : values };
  }) };
}
export function FilterBuilder({ value, exact, groups, onApply, onClear, onRemoveExact }: {
  ...
  const update = (index: number, patch: Partial<NativeFilterRule> & { matchUnknown?: boolean }) => setDraft(old => ({ ...old, rules: old.rules.map((rule, i) => i === index ? { ...rule, ...patch } : rule) }));
  ...
  function addRule() {
    setDraft(old => ({ ...(editing ? old : editDraft(value)), rules: [...(editing ? old.rules : editDraft(value).rules), { field: 'outbound', op: 'in', values: [], matchUnknown: false }] }));
  ...
  return <div className="space-y-3">
    ...
    {draft.rules.map((rule, index) => {
      ...
      return <fieldset key={index} disabled={pending} className="min-w-0 space-y-2 rounded-lg border p-3"><legend className="px-1 text-xs text-muted-foreground">{t("filterRule", { index: index + 1 })}</legend>
        <div className="flex flex-wrap gap-2"><select className={selectClass} aria-label={t("filterField", { index: index + 1 })} value={rule.field} onChange={event => update(index, { field: event.target.value as NativeFilterRule['field'], values: [], matchUnknown: false })}>{nativeFilterFields.map(field => <option key={field} value={field}>{t(field)}</option>)}</select>
          <select className={selectClass} aria-label={t("filterOp", { index: index + 1 })} value={rule.op} onChange={event => { const op = event.target.value as NativeFilterRule['op']; update(index, { op, values: ['in','notIn'].includes(op) ? rule.values : rule.values.slice(0, 1), matchUnknown: ['in','notIn'].includes(op) && rule.matchUnknown }); }}>{nativeFilterOperators.map(op => <option key={op} value={op}>{t('op_' + op)}</option>)}</select>
        ...
        {multi ? <><textarea className="min-h-16 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" aria-label={t("filterValue", { index: index + 1 })} placeholder={t("filterValuesHelp")} value={rule.values.join('\n')} onChange={event => update(index, { values: event.target.value ? event.target.value.split('\n') : [] })} />
          {!!choices.length && <details><summary className="cursor-pointer text-sm">{t("filterChoose")}</summary><div className="mt-2 flex max-h-40 flex-wrap gap-x-4 gap-y-2 overflow-auto">{choices.map(choice => <label key={choice} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={choice === '' ? rule.matchUnknown : rule.values.includes(choice)} onChange={event => update(index, choice === '' ? { matchUnknown: event.target.checked } : { values: event.target.checked ? [...rule.values.filter(text => text.trim() !== ''), choice] : rule.values.filter(v => v !== choice) })} />{choice || t("unknown")}</label>)}</div></details>}</> : <Input aria-label={t("filterValue", { index: index + 1 })} value={rule.values[0] || ''} onChange={event => update(index, { values: event.target.value ? [event.target.value] : [] })} />}
