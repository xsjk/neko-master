...
The GeoIP module exposes a country-only MMDB reader; the native SQL function registers on every querying connection, including export readers. It uses a bounded 50,000-IP in-memory cache and never calls an online geolocation service. Local lookups also classify historical facts directly, so no duplicate country aggregate table or asynchronous historical rewrite is needed. On the deployment snapshot, country and chain queries together took about 9 ms. Country assignments reflect the loaded database version.
